...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        ...
        self.total_human_actions_nums = 0
        self.human_actions_nums = 0
        self.q_diffs = []
        self.q_vars = []
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            if self.human_policy is not None:
                with th.no_grad():
                    ...
                    q_values = self.policy.q_net.forward(obs_tensor)
                    q_diff = (q_values.max() - q_values.min()).item()
                    q_var = q_values.var().cpu().item()
                    self.q_diffs.append(q_diff)
                    self.q_vars.append(q_var)

                    human_action = None
                    if self.use_baseline_ask == 'diff':
                        if q_diff > self.ask_threshold:
                            human_action = self.human_policy(obs_tensor)
                    elif self.use_baseline_ask == 'var':
                        if q_var < self.ask_threshold:
                            human_action = self.human_policy(obs_tensor)

                    if human_action is not None:
                        unscaled_action = human_action
                        self.human_actions_nums += 1
                        self.total_human_actions_nums += 1
    ...
    def _dump_logs(self) -> None:
        ...
        # ===================================== Ask-AC log start =====================================
        if self.human_policy is not None:
            logger.record("train/human_actions_num", self.human_actions_nums)
            logger.record("train/total_human_actions_num", self.total_human_actions_nums)
            logger.record("train/q_diff", np.mean(self.q_diffs))
            logger.record("train/q_var", np.mean(self.q_vars))
            self.human_actions_nums = 0
            self.q_diffs = []
            self.q_vars = []
        # ===================================== Ask-AC log end =====================================
