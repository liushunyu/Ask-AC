...
from stable_baselines3.common.torch_layers import BaseFeaturesExtractor, FlattenExtractor, MlpExtractor, NatureCNN, create_mlp, GIN, GCN
...
class BasePolicy(BaseModel):
    ...
    def predict(
        ...
        elif isinstance(observation, dgl.DGLGraph):
            observation = observation
        else:
        ...
        if isinstance(observation, dgl.DGLGraph):
            observation = observation.to(self.device)
        elif isinstance(observation, list) and isinstance(self.observation_space, Graph):
            observation = dgl.batch(observation).to(self.device)
        else:
...
class ActorCriticAskPolicy(ActorCriticPolicy):
    def __init__(
            self,
            observation_space: gym.spaces.Space,
            action_space: gym.spaces.Space,
        ...
        # Action distribution
        self.ask_action_dist = make_proba_distribution(gym.spaces.Discrete(n=2), use_sde=use_sde)
        self.standard_action_dist = make_proba_distribution(gym.spaces.Discrete(n=self.action_space.n - 1), use_sde=use_sde)
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        self.ask_action_net = self.ask_action_dist.proba_distribution_net(latent_dim=latent_dim_pi)
        self.standard_action_net = self.standard_action_dist.proba_distribution_net(latent_dim=latent_dim_pi)
        ...
        if self.ortho_init:
            ...
            module_gains = {
                self.features_extractor: np.sqrt(2),
                self.mlp_extractor: np.sqrt(2),
                self.ask_action_net: 0.01,
                self.standard_action_net: 0.01,
                self.value_net: 1,
            }
            for module, gain in module_gains.items():
                module.apply(partial(self.init_weights, gain=gain))
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        ask_distribution, standard_distribution = self._get_action_dist_from_latent(latent_pi)

        ask_actions = ask_distribution.get_actions(deterministic=deterministic)
        ask_log_prob = ask_distribution.log_prob(ask_actions)

        standard_actions = standard_distribution.get_actions(deterministic=deterministic)
        standard_log_prob = standard_distribution.log_prob(standard_actions)

        actions = ask_actions * (1 + standard_actions)
        log_prob = ask_log_prob + ask_actions * standard_log_prob
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        :return: Action distribution
        """
        mean_ask_actions = self.ask_action_net(latent_pi)
        mean_standard_actions = self.standard_action_net(latent_pi)

        return self.ask_action_dist.proba_distribution(action_logits=mean_ask_actions), self.standard_action_dist.proba_distribution(action_logits=mean_standard_actions)
    ...
    def evaluate_actions(self, obs: th.Tensor, actions: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        ask_actions = th.tensor(actions != 0, dtype=th.int64)
        standard_actions = actions - 1
        standard_actions[standard_actions == -1] = 0
        ask_distribution, standard_distribution = self._get_action_dist_from_latent(latent_pi)
        ask_log_prob = ask_distribution.log_prob(ask_actions)
        standard_log_prob = standard_distribution.log_prob(standard_actions)
        ...
        logits = th.zeros((batch_size, self.action_space.n), dtype=th.float32).to(self.device)

        probs[:, 0] = ask_distribution.distribution.probs[:, 0]
        logits[:, 0] = ask_distribution.distribution.logits[:, 0]

        probs[:, 1:] = ask_distribution.distribution.probs[:, 1].unsqueeze(1) * standard_distribution.distribution.probs
        logits[:, 1:] = ask_distribution.distribution.logits[:, 1].unsqueeze(1) + standard_distribution.distribution.logits

        min_real = th.finfo(logits.dtype).min
        logits = th.clamp(logits, min=min_real)
